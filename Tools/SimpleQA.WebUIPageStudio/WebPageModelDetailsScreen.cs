		...
		: Form
	{
		...
		private bool BuildPageModelClassCode (string pageModelFilePath)
		{
			try
			{
				...
				globalNamespace.Imports.Add (new CodeNamespaceImport ("SimpleQA.Framework.Abstractions"));
				globalNamespace.Imports.Add (new CodeNamespaceImport ("SimpleQA.Framework.Entities"));
				globalNamespace.Imports.Add (new CodeNamespaceImport ("SimpleQA.Framework.Selenium.WebUIControls"));
				...
				pageModelClass.BaseTypes.Add (new CodeTypeReference (typeof (SeleniumPageBase).Name));
							...
							.Add (
								new CodeAssignStatement (
									...
									new CodeMethodInvokeExpression (
										...
										nameof (SeleniumPageBase.CodeDomHelper.FindRadioButtonGroupByName),
							...
							.Add (
								new CodeAssignStatement (
									...
									new CodeMethodInvokeExpression (
										...
										nameof (SeleniumPageBase.CodeDomHelper.FindDropDownListByXPath),
							...
							.Add (
								new CodeAssignStatement (
									...
									new CodeMethodInvokeExpression (
										...
										nameof (SeleniumPageBase.CodeDomHelper.FindMultiSelectListBoxByXPath),
		...
		private CodeTypeMember DefinePropertyOutsideConstructor (UIControlHtmlTagMapperControl oneHtmlTagCustomControl)
		{
			...
			if (fieldType == nameof (RadioButtons))
			{
				assignmentExpression = $"{privateFieldName} = base.{ SeleniumPageBase.CodeDomHelper.FindRadioButtonGroupByName }(\"{oneHtmlTagCustomControl.HtmlTagInfo.Name}\");";
			}
			else if (fieldType == nameof (DropDownList))
			{
				assignmentExpression = $"{privateFieldName} = base.{SeleniumPageBase.CodeDomHelper.FindDropDownListByXPath}(\"{oneHtmlTagCustomControl.HtmlTagInfo.XPathInfo.XPathByDomPath}\");";
			}
			else if (fieldType == nameof (MultiSelectListBox))
			{
				assignmentExpression = $"{privateFieldName} = base.{SeleniumPageBase.CodeDomHelper.FindMultiSelectListBoxByXPath}(\"{oneHtmlTagCustomControl.HtmlTagInfo.XPathInfo.XPathByDomPath}\");";
			}
			else
			{
